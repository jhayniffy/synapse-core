...
fn test_set_not_ready() {
    let state = ReadinessState::new();
    state.set_not_ready();
    assert!(!state.is_ready(), "After set_not_ready, should not be ready");
    assert!(state.is_draining(), "After set_not_ready, should be draining");
}
...
fn test_set_ready() {
    ...
    assert!(!state.is_draining(), "After set_ready, should not be draining");
}

#[test]
fn test_drain_timeout_custom() {
    let state = ReadinessState::with_drain_timeout(60);
    assert_eq!(state.drain_timeout().as_secs(), 60, "Drain timeout should be 60 seconds");
}

#[test]
fn test_drain_timeout_default() {
    let state = ReadinessState::new();
    assert_eq!(state.drain_timeout().as_secs(), 30, "Default drain timeout should be 30 seconds");
}
...
fn test_start_drain() {
    ...
    assert_eq!(timeout.as_secs(), 45, "start_drain should return configured timeout");
...
async fn test_wait_for_drain() {
    ...
    assert!(!state.is_ready(), "Should still be not ready after drain wait");
}
